/**
 * R2 client using the S3-compatible API with Access Key ID + Secret.
 * Reusing the legacy R2 API token shared with another app — the token
 * has the deanst bucket added to its scope, so the same credentials
 * cover both buckets.
 *
 * Required env vars:
...
function client(): S3Client {
  ...
  if (!endpoint) throw new Error("R2 not configured: missing R2_ACCOUNT_ID");
